...
type t = {
  ...
  mutable lastStepCommand : t -> bool Lwt.t;
...
module Event = struct
  ...
  let emit (type a) mi2 (event : a t) (arg : a) =
    let args =
      match event with
      | Message         -> List.map Obj.magic [ (fst arg); (snd arg) ]
      | DebugReady      -> []
      | Quit            -> []
      | ExecAsyncOutput -> [ Obj.magic arg ]
      | ExitedNormally  -> [ Obj.magic arg ]
      | SignalStop      -> [ Obj.magic arg ]
      | Stopped         -> [ Obj.magic arg ]
      | Running         -> [ Obj.magic arg ]
      | StepEnd         -> [ Obj.magic arg ]
      | StepOutEnd      -> [ Obj.magic arg ]
      | ThreadCreated   -> [ Obj.magic arg ]
      | ThreadExited    -> [ Obj.magic arg ]
    in
  ...
  let wrapListener (type a) (event : a t) (listener : a -> unit) =
    (fun ~args ->
       ...
       | Message,    a1 :: a2 :: _ -> listener ( Obj.magic a1,
                                                 Obj.magic a2)
       | DebugReady,             _ -> listener ()
       | Quit,                   _ -> listener ()
       | ExecAsyncOutput, arg :: _ -> listener (Obj.magic arg)
       | ExitedNormally,  arg :: _ -> listener (Obj.magic arg)
       | SignalStop,      arg :: _ -> listener (Obj.magic arg)
       | Stopped,         arg :: _ -> listener (Obj.magic arg)
       | Running,         arg :: _ -> listener (Obj.magic arg)
       | StepEnd,         arg :: _ -> listener (Obj.magic arg)
       | StepOutEnd,      arg :: _ -> listener (Obj.magic arg)
       | ThreadCreated,   arg :: _ -> listener (Obj.magic arg)
       | ThreadExited,    arg :: _ -> listener (Obj.magic arg)
...
and init mi2 gdbPath procEnv noDebug
  ...
  let procEnv =
    let env = Dict.of_seq @@ Util.get_env () in
    Dict.union (fun _n _v1 v2 ->
      Some v2) env procEnv
  in
  mi2.gdbPath <- gdbPath;
  mi2.procEnv <- procEnv;
