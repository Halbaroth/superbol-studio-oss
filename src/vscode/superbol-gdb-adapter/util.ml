...
let get_env () = 
  let env = Array.to_seq @@ Unix.environment () in
  Seq.map (fun s ->
    match String.index_from s 0 '=' with
    | exception Not_found -> 
        (* According to POSIX.1-2017, the binding must contain an equal sign 
           and the name variable cannot contain a egal sign. *)
        assert false
    | i ->
        let len = String.length s in
        let name = String.sub s 0 i in
        let value = String.sub s (i + 1) (len - i - 1) in
        (name, value)) env
